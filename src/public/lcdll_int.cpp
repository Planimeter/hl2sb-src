...
static int engine_DoesBoxTouchAreaFrustum (lua_State *L) {
  lua_pushboolean(L, engine->DoesBoxTouchAreaFrustum(luaL_checkvector(L, 1), luaL_checkvector(L, 2), luaL_checkinteger(L, 3)));
  return 1;
}
...
static int engine_GetLightForPoint (lua_State *L) {
  lua_pushvector(L, engine->GetLightForPoint(luaL_checkvector(L, 1), luaL_checkboolean(L, 2)));
  return 1;
}

static int engine_GetLightForPointFast (lua_State *L) {
  lua_pushvector(L, engine->GetLightForPointFast(luaL_checkvector(L, 1), luaL_checkboolean(L, 2)));
  return 1;
}
...
static int engine_IsBoxInViewCluster (lua_State *L) {
  luaL_checkinteger(L, engine->IsBoxInViewCluster(luaL_checkvector(L, 1), luaL_checkvector(L, 2)));
  return 1;
}

static int engine_IsBoxVisible (lua_State *L) {
  luaL_checkinteger(L, engine->IsBoxVisible(luaL_checkvector(L, 1), luaL_checkvector(L, 2)));
  return 1;
}
