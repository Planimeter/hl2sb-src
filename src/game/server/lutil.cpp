...
static int luasrc_UTIL_ClearTrace (lua_State *L) {
  trace_t trace = (trace_t &)luaL_checktrace(L, 1);
...
static int luasrc_UTIL_ClipPunchAngleOffset (lua_State *L) {
  ...
  CBasePlayer *pPlayer = luaL_checkplayer(L, 2);
  UTIL_ClipPunchAngleOffset(in, pPlayer->m_Local.m_vecPunchAngle, luaL_checkangle(L, 3));
  lua_pushangle(L, in);
  return 1;
}
...
static int luasrc_UTIL_SnapDirectionToAxis (lua_State *L) {
  Vector direction = luaL_checkvector(L, 1);
  UTIL_SnapDirectionToAxis(direction, luaL_optnumber(L, 2, 0.002000));
